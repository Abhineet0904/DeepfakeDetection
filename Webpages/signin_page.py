...
myAuth = pyrebase.initialize_app(json.loads(os.getenv("FIREBASE_CONFIG"))).auth()
db = firestore.client()
...
def login(email, password) :
    ...
    if user :
        ...
        if user_doc :
            st.session_state.user_data = [myAuth.current_user["localId"]] + list(user_doc.to_dict().values())
...
def signup(email, password, username) :
    ...
    if new_user :
        ...
        st.session_state.user_data = [myAuth.current_user["localId"], email, username]
        st.session_state["logged_in"] = True
        st.toast("**You can continue to the deepfake detection engine.**")
